...
export default class AddonMessagesSearchPage implements OnDestroy {
    ...
    protected backendLimitFromContacts = 0;
    protected backendLimitFromNonContacts = 0;
    protected backendLimitFromMessages = 0;
    ...
    async search(query: string, loadMore?: 'contacts' | 'noncontacts' | 'messages', infiniteComplete?: () => void): Promise<void> {
        ...
        if (!loadMore) {
            this.backendLimitFromContacts = 0;
            this.backendLimitFromNonContacts = 0;
            this.backendLimitFromMessages = 0;
        }

        const terms = query.trim().split(/\s+/);
        const backendQuery = terms[0];
        const localFilterTerms = terms.length > 1 ? terms.slice(1).map(t => t.toLowerCase()) : [];
        ...
        if (!loadMore || loadMore == 'contacts' || loadMore == 'noncontacts') {
            ...
            if (loadMore == 'contacts') {
                limitFrom = this.backendLimitFromContacts;
                this.contacts.loadingMore = true;
            } else if (loadMore == 'noncontacts') {
                limitFrom = this.backendLimitFromNonContacts;
                this.nonContacts.loadingMore = true;
            }

            promises.push(
                AddonMessages.searchUsers(backendQuery, limitFrom, limitNum).then((result) => {
                    if (!loadMore || loadMore == 'contacts') {
                        this.backendLimitFromContacts += result.contacts.length;
                        newContacts = result.contacts;
                        if (localFilterTerms.length > 0) {
                            newContacts = newContacts.filter(contact => {
                                const name = contact.fullname.toLowerCase();

                                return localFilterTerms.every(term => name.includes(term));
                            });
                        }
                    ...
                    if (!loadMore || loadMore == 'noncontacts') {
                        this.backendLimitFromNonContacts += result.nonContacts.length;
                        newNonContacts = result.nonContacts;
                        if (localFilterTerms.length > 0) {
                            newNonContacts = newNonContacts.filter(contact => {
                                const name = contact.fullname.toLowerCase();
        ...
        if (!loadMore || loadMore == 'messages') {
            ...
            if (loadMore == 'messages') {
                limitFrom = this.backendLimitFromMessages;
                this.messages.loadingMore = true;
            }

            promises.push(
                AddonMessages.searchMessages(backendQuery, undefined, limitFrom).then((result) => {
                    this.backendLimitFromMessages += result.messages.length;
                    newMessages = result.messages;
                    if (localFilterTerms.length > 0) {
                        newMessages = newMessages.filter(message => {
                            const name = message.fullname.toLowerCase();
                            const text = message.lastmessage ? message.lastmessage.toLowerCase() : '';

                            return localFilterTerms.every(term => name.includes(term) || text.includes(term));
                        });
                    }

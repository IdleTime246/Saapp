...
export class CoreCourseFormatComponent implements OnInit, OnChanges, OnDestroy {
    ...
    protected async treatSections(sections: CoreCourseSectionToDisplay[]): Promise<void> {
        ...
        if (!this.loaded) {
            ...
            // Force "All sections" if available.
            const allSections = sections.find(s => s.id === this.allSectionsId);
            if (allSections) {
                section = allSections;
            }
    ...
    protected async isAllSectionsPreferred(): Promise<boolean> {
        const showAllSections =
            await this.currentSite?.getLocalSiteConfig<number>(`${CORE_COURSE_ALL_SECTIONS_PREFERRED_PREFIX}${this.course.id}`, 1);
